...
const EnrichmentPatchSchema = z.object({
  ...
  emotional_map: opt(z.array(z.any())),
  sensory_triggers: opt(z.array(z.string().min(1))),
  conversation_triggers: opt(z.array(z.string().min(1))),
  client_archetypes: opt(z.array(z.any())),
  pricing: opt(z.record(z.string(), z.any())),
  confidence_score: opt(z.number().int().min(0).max(100)),
  emotion_confidence: opt(z.number().int().min(0).max(100)),
